    ...
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#fff', minHeight: '100vh', paddingBottom: '160px' }}>
      <header style={{ padding: '15px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', position: 'sticky', top: 0, backgroundColor: '#fff', zIndex: 10 }}>
        ...
        <div style={{ color: '#ff7920', fontWeight: 'bold' }}>
          <ShoppingCart size={20} style={{display:'inline', marginRight: '5px'}} /> {cart.length}
        </div>
      </header>
          ...
          <div key={item.id} style={{ display: 'flex', backgroundColor: '#fff', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', marginBottom: '15px', border: '1px solid #f0f0f0' }}>
            ...
            <div style={{ padding: '10px', flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {item.half && (
                  <button onClick={()=>addToCart(item, 'Half', item.half!)} style={{ flex: 1, padding: '6px', border: '1px solid #ff7920', borderRadius: '8px', background: 'none', color: '#ff7920', fontSize: '11px', fontWeight: 'bold' }}>
                    Half ₹{item.half}
                  </button>
                )}
                {item.full && (
                  <button onClick={()=>addToCart(item, 'Full', item.full)} style={{ flex: 1, padding: '6px', background: '#ff7920', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '11px', fontWeight: 'bold' }}>
                    Full ₹{item.full}
                  </button>
                )}
              </div>
      ...
      {cart.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, width: '100%', backgroundColor: '#fff', padding: '15px', borderTop: '1px solid #eee', boxSizing: 'border-box', boxShadow: '0 -4px 10px rgba(0,0,0,0.05)' }}>
          <input 
            type="text" 
            placeholder="Delivery Address..." 
            onChange={(e)=>setAddress(e.target.value)} 
            style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #eee', marginBottom: '10px', boxSizing: 'border-box' }} 
          />
          <button 
            onClick={() => window.open(`https://wa.me/919699343711?text=${encodeURIComponent(`New Order:\n${cart.map(i=>`${i.name}(${i.type}) x ${i.qty}`).join('\n')}\nTotal: ₹${total}\nAddress: ${address}`)}`)} 
            style={{ width: '100%', backgroundColor: '#ff7920', color: '#fff', padding: '15px', borderRadius: '12px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}
          >
            Order Now (₹{total})
          </button>
        </div>
