    ...
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#fff', minHeight: '100vh', paddingBottom: '160px' }}>
      ...
      {cart.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, width: '100%', backgroundColor: '#fff', padding: '15px', borderTop: '1px solid #eee', boxSizing: 'border-box' }}>
          <input type="text" placeholder="Address..." onChange={(e)=>setAddress(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #eee', marginBottom: '10px' }} />
          <button onClick={() => window.open(`https://wa.me/919699343711?text=${encodeURIComponent(`Order: ${cart.map(i=>`${i.name}(${i.type})x${i.qty}`).join(', ')}\nTotal: ₹${total}\nAddress: ${address}`)}`)} style={{ width: '100%', backgroundColor: '#ff7920', color: '#fff', padding: '15px', borderRadius: '12px', fontWeight: 'bold', border: 'none' }}>
            Order Now (₹{total})
          </button>
        </div>
      )}
